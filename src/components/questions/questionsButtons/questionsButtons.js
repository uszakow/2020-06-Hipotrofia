...
function QuestionsButtons(props) {
    ...
    return (
        <div>
        <button className='questions_questionsButtons_button' onClick={showHide}>{question}<img src={Vector} className='rotate' id={'img' + question}/></button>
        <div id={question} className='hide'>{answer}</div> {/*zmiana nazwy klasy na "questions_questionsButtons_hide" powoduje, że button nie działa, nie wiem dlaczego*/}
        </div>
    )
}
